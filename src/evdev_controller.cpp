...
EvdevController::EvdevController(const std::string& filename,
  ...
  if (m_grab) {  // grab the device, so it doesn't broadcast events into the
                 // wild
...
void EvdevController::set_rumble_real(uint8_t left, uint8_t right) {
  // not implemented
}

void EvdevController::set_led_real(uint8_t status) {
  // not implemented
}

void EvdevController::upload(const struct ff_effect& effect) {
  struct ff_effect cpy = effect;
  if (effect_ids.find(effect.id) == effect_ids.end()) {
    cpy.id = -1;
  }
